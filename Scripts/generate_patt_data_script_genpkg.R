...
source("./Scripts/run_patt_scenario_genpkg.R")
source("./Modules/p_val_add.R")
source("./Modules/equity_metrics_miao.R")

TA_World <- data %>% filter(RANDASSIGN==1)
TA_size <- 2000
total_seeds <- 15 #number of total scenarios 15
total_success <- 30 #number of inner bootstraps 30
IPF_maxiter <- 100
step_size <- 500
...
packages <- c("dplyr", "ipfr", "MatchIt", "marginaleffects", "generalize", "tableone", "survival")
...
filename <- "PATT_Summary_extrabias_genpkg_treat2k.csv"
