...
run_one_patt_scenario <- function(seed_value, CC_Size){
  ...
  #create the propensity score model - propensity to be treated, normal propensity score
  M <- get_propensity_model(do.call("rbind", list(TA %>% mutate(inTreat = 1),
                                                  CC %>% mutate(inTreat = 0),
                                                  Biased_EC_world %>% mutate(inTreat = 0)))) #push within internal loop in specific scenario
  ...
  while (success_counter <= total_success){ ## Inner Bootstraps
    ...
    for (n_plus_sum in 0:4){ #for each inner_seed value I'll try 5 times to get a pop without missing subcategory
      ...
      unmatched_EC <- Biased_EC_world #pseudo-random
