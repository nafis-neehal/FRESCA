...
while(seed_counter <= total_seed) {
  ...
  TA <- sample_n(data %>% filter(RANDASSIGN==1), 2000)
  CC <- sample_n(data %>% filter(RANDASSIGN==0), 250)
  external_population <- setdiff(data, rbind(TA, CC))
  EC_world <- external_population %>% filter(RANDASSIGN==0)
  
  #check for various EC size
  EC_size_list <- c(0, 500, 1000, 2000, nrow(EC_world))
  total_success <- length(EC_size_list)
  
  #step: 6a(i)
  W_EC_ipf <- get_ECWorld_Bias_weights(dat = EC_world, maxIter = 5000)
  ...
  while(success_counter <= total_success){
    ...
    #step: 6a(i)(3)
    W_TA_ipf <- get_IPF_weights(dat = TA, maxIter = 5000)
    W_HC_ipf <- get_IPF_weights(dat = HC, maxIter = 5000)
    ...
    for (n in 1:20) {
      ...
      set.seed(n)
      #cat(paste0("Seed has been set to ", n, " at Level 2\n"))
      ...
      LDM_summary <- get_LDM_summary(TA, CC, EC_ipf_sample, EC, HC, TA_IPF_sample, HC_IPF_sample)
  ...
  df_ATT_summary_loop1 <- data.frame(seed = seed_value,
                                     ...
                                     Base_Diff = abs(unlist(ATT_list_loop1)[1] - unlist(ATT_list_loop1)),
...
write.csv(df0,
          "./Data/Results/PATT_m6/EC_Vary_Size/2000TA_250CC_varyECSize_test.csv")


# Optional Plots

plotdata1 <- df0 %>% group_by(seed)

#colors_list <- c("red", "black", "green", "blue", "cyan", "magenta", "yellow", "orange", "grey", "purple")
ggplot(plotdata1, aes(x=EC_Sample_Size, y=Mean_Patt, group=seed, color=seed)) + 
  geom_line() + geom_point() + 
  geom_hline(yintercept = -14.8, linetype="dashed", color="red") + 
  geom_hline(yintercept = -15.2, linetype="dashed", color="green")

ggplot(plotdata1, aes(x=EC_Sample_Size, y=Var_Patt, group=seed, color=seed)) + 
  geom_line() + geom_point()
