...
while(success_counter <= total_success){
  ...
  LDM_summary <- get_LDM_summary(TA, CC, EC_candidate, EC, HC, TA_IPF_sample, HC_IPF_sample)
...
write.csv(as.data.frame(unlist(ATT_list)) %>% rename(PATT_m2 = 'unlist(ATT_list)'), 
          "./Data/Results/PATT_m2.csv")
...
final_dat <- datatable(ldm_mean, options = list(scrollX = T)) %>% 
  formatStyle(c('LDM_TA', "LDM_CC", "LDM_EC_candidate", "LDM_EC", "LDM_HC", "LDM_IPF_TA", "LDM_IPF_HC"),
  ...
  formatRound(c('LDM_TA', "LDM_CC", "LDM_EC_candidate", "LDM_EC", "LDM_HC", "LDM_IPF_TA", "LDM_IPF_HC"), digits = 3)
