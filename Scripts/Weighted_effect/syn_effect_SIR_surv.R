...
res <- coxph(Surv(y, failed)~Tr, data = trial_data, weights = trial_data$new_weights)
summary(res)
...
#new weights are correctly generated
res <- coxph(Surv(y, failed)~Tr, data = hybrid_data, weights = hybrid_data$new_weights)
summary(res)
