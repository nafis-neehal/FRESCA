...
run_one_scenario <- function(seed_value, CC_Size){
  ...
  while (success_counter <= total_success){ ## Inner Bootstraps
    ...
    for (n_plus_sum in 0:4){ #for each inner_seed value I'll try 5 times to get a pop without missing subcategory
      ...
      EC_ipf_sample <- EC_world[sample(seq_len(nrow(EC_world)), 
                                       size = nrow(EC_world), 
                                       replace = TRUE, prob = W_EC_bias_ipf),] #unmatched EC
      
      matched_EC_df <- get_matched_HC(rbind(TA, EC_ipf_sample)) #with weights
      matched_EC <- matched_EC_df %>% select(MASKID:SEATSYS) #matched EC data
      
      unmatched_HC <- rbind(CC, EC_ipf_sample %>% select(MASKID:SEATSYS)) #unmatched HC
      
      matched_HC <- rbind(CC, matched_EC) #matched HC
      matched_HC_weights <- c(rep(1,nrow(CC)), matched_EC_df$weights)
    ...
    #<- Collect TA_HC_prop
    ATT_TA_HC_prop_list <- ATT_TA_HC_prop_list + get_ATT_with_propensity(rbind(TA, unmatched_HC))
    
    #Apply IPF on TA and HC + Bootstrap
    W_HC_ipf <- get_IPF_weights(dat = unmatched_HC, maxIter = IPF_maxiter) #no propensity
    ...
    unmatched_HC_IPF_sample <- unmatched_HC[sample(seq_len(nrow(unmatched_HC)), 
                                                   size = nrow(TA), replace = TRUE, prob = W_HC_ipf),]
    
    set.seed(inner_seed)
    matched_HC_IPF_sample <- matched_HC_df[sample(seq_len(nrow(matched_HC_df)), 
                                                  size = nrow(TA), replace = TRUE, prob = W_matched_HC_ipf),]
    matched_HC_IPF_sample_dat <- matched_HC_IPF_sample %>% select(MASKID:SEATSYS)
    ...
    ATT_TA_HC_both_list <- ATT_TA_HC_both_list + get_ATT_val(rbind(TA_IPF_sample, 
                                                                   matched_HC_IPF_sample_dat), 
                                                             weights = c(rep(1, nrow(TA_IPF_sample)),
                                                                         matched_HC_IPF_sample$weights)) #matched HC
...
Final_PATT_Summary <- foreach(seed_value=seq(1,total_seeds,1), .combine = 'rbind', .packages = packages, 
                              .options.snow = opts) %:%
...
filename <- "PATT_Summary_Seed20_nested_optimized_gfr_v13.csv"
