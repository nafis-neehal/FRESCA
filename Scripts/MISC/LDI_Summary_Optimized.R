...
run_one_scenario <- function(seed_value, s, TA, W_TA_ipf, LDM_TA, CC_World, EC_world,
...
for (seed_value in 1:total_seeds){ #loops 20 times
  ...
  Seed_LDM_Summary <- foreach (size=CC_size_list, .combine = "rbind", .packages = packages, .options.snow = opts) %dopar%{
    run_one_scenario(seed_value, size, TA, W_TA_ipf, LDM_TA, CC_World, EC_world,
                     CC_size_list, EC_size_list, W_EC_ipf)
  }
  Final_LDM_Summary <- rbind(Final_LDM_Summary, Seed_LDM_Summary)
}
