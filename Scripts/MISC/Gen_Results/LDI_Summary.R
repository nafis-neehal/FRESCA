...
get_matched_HC <- function(population){
  ...
  matched_HC <- matched_data %>% filter(RANDASSIGN == 0) %>% select(MASKID:SEATSYS)
...
for (seed_value in 1:total_seeds){ #loops 20 times
  ...
  i <- 1
  for (s in CC_size_list){ #loops 21 times
    ...
    while (success_counter <= total_success){ #loops 10 times
      ...
      for (n_plus_sum in 0:4){ #for each inner_seed value I'll try 5 times to get a pop without missing subcategory
        ...
        matched_HC <- get_matched_HC(rbind(TA, HC)) #matched
      ...
      set.seed(inner_seed)
      matched_HC_IPF_sample <- matched_HC[sample(seq_len(nrow(matched_HC)), size = nrow(TA), 
                                                 replace = TRUE, prob = W_matched_HC_ipf),]
      ...
      LDM_HC_IPF <- get_LDM_from_count(HC_IPF_sample) %>% select(LDM) %>% rename(LDM_HC_IPF = LDM)
    ...
    #add dataframe
    row <- as.data.frame(list(seed_value, s, VAR, Level,
                              abs(LDM_TA),
...
filename <- "LDM_Summary_Seed20_withReplace_new3_nosbp.csv"
#write.csv(LDM_Summary, paste(directory, filename, sep = ""), row.names = FALSE)

LDM_Summary <- read.csv(paste(directory, filename, sep = ""))

Inf_replace <- max(LDM_Summary %>% mutate_all(function(x) ifelse(is.infinite(x), -1, x)) %>% 
                     select(LDM_CC:LDM_HC_Prop_IPF)) + 0.5
LDM_Summary_Inf <- LDM_Summary
LDM_Summary_Inf[sapply(LDM_Summary_Inf, is.infinite)] <- Inf_replace

mean_summary <- LDM_Summary_Inf %>% 
  group_by(CC_Size, Level) %>% 
  summarise(mean(LDM_TA), 
  ...
  rename(LDM_TA = "mean(LDM_TA)",
         LDM_CC = "mean(LDM_CC)",
         LDM_HC_Prop = "mean(LDM_HC_Prop)",
         LDM_HC_IPF = "mean(LDM_HC_IPF)",
         LDM_TA_Prop_IPF = "mean(LDM_TA_Prop_IPF)",
         LDM_HC_Prop_IPF = "mean(LDM_HC_Prop_IPF)")
...
return_plot <- function(var, level){
  ...
  p <- mean_summary %>% filter(Level==level) %>%
    ...
    labs(x="CC_Size", y="Mean LDM Estimate", caption = cap)
  return (p)
}
