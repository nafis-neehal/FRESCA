...
run_one_scenario <- function(seed_value, s, TA, W_TA_ipf, CC_World, EC_world,
                             CC_size_list, EC_size_list, W_EC_ipf){
  ...
  while (success_counter <= total_success){ ## Inner Bootstraps
    ...
    for (n_plus_sum in 0:4){ #for each inner_seed value I'll try 5 times to get a pop without missing subcategory
      ...
      flag <- check_subgroup_counts(age_sbgrps = 2, gender_sbgrps = 2, race_sbgrps = 5, 
                                    frs_sbgrps = 2, gfr_sbgrps = 2, TA = TA, HC = matched_HC)
      if (flag==1){
        break
      }
      else{
    ...
    ATT_TA_HC_list <- ATT_TA_HC_list + get_ATT_val(rbind(TA, HC), weights = NULL)
    
    #<- Collect TA_HC_prop
    ATT_TA_HC_prop_list <- ATT_TA_HC_prop_list + get_ATT_with_propensity(rbind(TA, HC))
    ...
    ATT_TA_HC_IPF_list <- ATT_TA_HC_IPF_list + get_ATT_val(rbind(TA_IPF_sample, HC_IPF_sample), weights = NULL) #Unmatched data
    ATT_TA_HC_both_list <- ATT_TA_HC_both_list + get_ATT_val(rbind(TA_IPF_sample, matched_HC_IPF_sample), 
                                                             weights = c(rep(1, nrow(TA_IPF_sample)),matched_HC_IPF_weights)) #matched HC
...
for (seed_value in 1:total_seeds){
  ...
  print(paste("Scenario:", seed_value,"started"))
  ...
  pb <- txtProgressBar(max = length(CC_size_list), style = 3)
  progress <- function(n) setTxtProgressBar(pb, n)
  opts <- list(progress = progress)
  
  Seed_PATT_Summary <- foreach (size=CC_size_list, .combine = "rbind", 
                                .packages = packages, .options.snow = opts) %dopar%{
    run_one_scenario(seed_value, size, TA, W_TA_ipf, CC_World, EC_world,
                     CC_size_list, EC_size_list, W_EC_ipf)
  }
  Final_PATT_Summary <- rbind(Final_PATT_Summary, Seed_PATT_Summary)
}
...
filename <- "PATT_Summary_Seed20_optimized_newvars_v2.csv"
...
table1(~ CVDPOINTS + GFRESTIMATE+ FPG + TC + CVDHISTORY +  SERUMCREAT | group, 
       data=population)
