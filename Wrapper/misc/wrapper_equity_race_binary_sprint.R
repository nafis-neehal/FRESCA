...
run_main_simulation <- function(num_seed){
  ...
  for (i in 1:num_seed){
    ...
    #### create dummies of RCT, Biased EC and target_data_count
    target_data_count_dummy <- create_dummies(target_data_count, column = "Race_or_Ethnicity")
    target_data_count <- target_data_count_dummy[, c(2,3,5,6,7,8,9,4)]
    ...
    # synthetic_controls <- get_matched_controls_with_dummies(method_name = "top_n_propensity",
    ...
    #recode race before calculating LDM
    TR_ldm_summary <- get_LD_nth_level(recode_race(hybrid_RCT) %>% filter(RANDASSIGN==1), recode_race(target_data_count), 
                                       NULL,
    ...
    #recode race before calculating LDM
    CN_ldm_summary <- get_LD_nth_level(recode_race(hybrid_RCT) %>% filter(RANDASSIGN==0), recode_race(target_data_count), 
                                       NULL,
                                      var_name_list, var_subgroup_list, 1) #try 1 and 3 for now
