...
run_main_simulation <- function(num_seed, pop){
  ...
  for (i in 1:num_seed){
    ...
    if (zero_cc==F){
      only_equity_adjusted_CC_sample <- sample_n(RCT, size = nrow(RCT), replace = T, weight = c(tr_adjusted_weights, cc_adjusted_weights))
      only_equity <- c(only_equity, exp(get_adjusted_effect(only_equity_adjusted_CC_sample, outcome_days_column, outcome_status_column, NULL)))
    }
    else{
    ...
    only_equity_adjusted_hybrid_sample <- sample_n(hybrid_RCT, size = nrow(hybrid_RCT), replace = T, weight = c(tr_adjusted_weights, hybrid_adjusted_weights))
    both <- c(both, exp(get_adjusted_effect(only_equity_adjusted_hybrid_sample, outcome_days_column, outcome_status_column, NULL)))
...
for (CC_Size in CC_Size_list){
  ...
  write.csv(do.call("rbind", list(result1)),
            paste('./Data/Results/M6/ALLHAT_Results/PHR_ipfs/Matchit3/',outcome_name,'_full_', file_string_variable, sep = ""),
            row.names=F) 
  write.csv(do.call("rbind", 
                    list(summary_df1)) %>% select(combination, values, CC_only:BOTH),
            paste('./Data/Results/M6/ALLHAT_Results/PHR_ipfs/Matchit3/',outcome_name,'_summary_', file_string_variable, sep= ""),
            row.names=F) 
...
#   # CC
#   set.seed(1)
#   CC_World <- sample_n(g1 %>% filter(RANDASSIGN==0), TA_Size)
#   
#   # EC
#   external_population <- setdiff(g1, rbind(TA_World, CC_World)) 
...
#   
#   demo <- table1(~ Age_Group + Gender + Race_or_Ethnicity | group, 
#                  data=population, overall=F)
#   print(demo)
#   directory <- "./Data/Results/M6/ALLHAT_Results/Table1/"
