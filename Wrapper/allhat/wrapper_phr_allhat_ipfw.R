...
for (CC_Size in CC_Size_list){
  ...
  write.csv(do.call("rbind", list(result1)),
            paste('./Data/Results/M6/ALLHAT_Results/PHR_ipfw/Matchit3/',outcome_name,'_full_', file_string_variable, sep = ""),
            row.names=F) 
  write.csv(do.call("rbind", 
                    list(summary_df1)) %>% select(combination, values, CC_only:BOTH),
            paste('./Data/Results/M6/ALLHAT_Results/PHR_ipfw/Matchit3/',outcome_name,'_summary_', file_string_variable, sep= ""),
            row.names=F) 
...
#                  Prior_HypTreat+ SBP+ DBP+ Smoker+Had_MIS+ Had_CRV+Had_ASCVD+
#                  Had_MajorST+Had_T2D+Had_HDLC+ Had_LVH_ELCT+Had_LVH_ECHO+Has_CHD+ BMI| group,
