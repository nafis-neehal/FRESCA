...
CC_Size_list <- c(0, 1000, 2000, 3000, 4000, 5000, 6000)
for (CC_Size in CC_Size_list){
  ...
  # 
  # start <- Sys.time()
  # summary2 <- run_main_simulation(num_seed = num_seed, pop = population)
  # end <- Sys.time()
  # paste("Total time elapsed:", end-start)
  # summary2$combination <- combination
  ...
  write.csv(do.call("rbind", list(summary1)),
            paste('./Data/Results/M6/ALLHAT_Results/Equity/Matchit3/',eq_str,'_full_', file_string_variable, sep = ""),
            row.names=F)
}
