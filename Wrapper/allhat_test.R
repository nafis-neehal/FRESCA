...
# run_simulation <- function(pop, trgrp){
#   
#   i <- 1
...
#   remove(all_data)
#   
#   RCT_tr11 <- RCT %>% filter(RANDASSIGN==1) %>% select(Gender, Age_Group, Race_or_Ethnicity) %>% mutate(grp = trgrp)
#   RCT_cn1 <- RCT %>% filter(RANDASSIGN==0) %>% select(Gender, Age_Group, Race_or_Ethnicity) %>% mutate(grp = "G2:Cn(Ch)")
#   biased1 <- BIASED_EC %>% select(Gender, Age_Group, Race_or_Ethnicity) %>% mutate(grp = "G3:Biased_EC")
#   tar1 <- target_data %>% select(Gender, Age_Group, Race_or_Ethnicity) %>% mutate(grp = "G4:Target")
#   
#   fullpop <- do.call("rbind", list(RCT_tr11, RCT_cn1, biased1, tar1))
#   
#   return(fullpop)
# 
# }
...
# IPF_maxiter <- 100
# fullpop1 <- run_simulation(pop=g1, trgrp = "G1:Trt (AM)")
...
# SC_Size <- TA_Size - CC_Size
# IPF_maxiter <- 100
# fullpop2 <- run_simulation(pop=g2, trgrp = "G1:Trt (LI)")
