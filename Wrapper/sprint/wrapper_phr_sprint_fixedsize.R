...
# TA_Size_list <- c(2000, 2500, 3000, 3500)
# CC_Size_list <- c(2000, 1500, 1000, 500)
# SC_Size_list <- c(0, 1000, 2000, 3000)
...
for (i in seq(1, length(TA_Size_list), 1)){
  ...
  write.csv(result1,
            paste('./Data/Results/M6/New_SPRINT_Results/Samecost/trial_size_6k/Matchit3/',outcome_name,'_samecost_full_', file_string_variable, sep = ""),
            row.names=F) 
  write.csv(summary_df1 %>% select(combination, values, CC_only:BOTH),
            paste('./Data/Results/M6/New_SPRINT_Results/Samecost/trial_size_6k/Matchit3/',outcome_name,'_samecost_summary_', file_string_variable, sep= ""),
            row.names=F) 
}
